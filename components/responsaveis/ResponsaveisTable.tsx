...
export default function ResponsaveisTable() {
  ...
  const filteredResponsaveis = sortedResponsaveis.filter(responsavel => 
    responsavel.ds_nome.toLowerCase().includes(searchQuery.toLowerCase()) ||
    responsavel.ds_email.toLowerCase().includes(searchQuery.toLowerCase()) ||
    responsavel.nm_telefone.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleDeleteResponsavel = (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir este responsável?')) {
      setResponsaveis(responsaveis.filter(responsavel => responsavel.id_responsavel !== id));
    }
  };
  ...
  const handleSaveResponsavel = (responsavel: Responsavel) => {
    if (selectedResponsavel) {
      ...
      setResponsaveis(responsaveis.map(r => r.id_responsavel === responsavel.id_responsavel ? responsavel : r));
    } else {
      // Add new responsavel
      ...
      <div className="flex-1 overflow-auto">
        <Table>
          <TableHeader className="bg-gray-50">
            <TableRow>
              <TableHead className="cursor-pointer" onClick={() => handleSort('ds_nome')}>
                <div className="flex items-center">
                  Nome
                  <ArrowUpDown className="ml-2 h-4 w-4" />
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('ds_email')}>
                <div className="flex items-center">
                  Email
                  <ArrowUpDown className="ml-2 h-4 w-4" />
                </div>
              </TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('nm_telefone')}>
                <div className="flex items-center">
                  Telefone
                  <ArrowUpDown className="ml-2 h-4 w-4" />
                </div>
          ...
          <TableBody>
            {filteredResponsaveis.length === 0 ? (
              ...
              filteredResponsaveis.map((responsavel) => (
                <TableRow key={responsavel.id_responsavel} className="hover:bg-gray-50">
                  <TableCell className="font-medium">{responsavel.ds_nome}</TableCell>
                  <TableCell>{responsavel.ds_email}</TableCell>
                  <TableCell>{responsavel.nm_telefone}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        onClick={() => handleDeleteResponsavel(responsavel.id_responsavel)}
